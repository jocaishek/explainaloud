...
export function StreakToast() {
  ...
  /* Held open while somebody is reading it.
   *
   * A card that vanishes on a timer under a pointer that is resting on it, or
   * under a caret that has just tabbed into its link, is a card that takes its
   * own content away mid-sentence. The clock restarts when they leave. */
  const hold = useCallback(() => clear(), [clear]);

  const release = useCallback((big: boolean) => {
    if (timer.current) clearTimeout(timer.current);
    timer.current = setTimeout(
      () => setShown(null),
      big ? HOLD_MS_BIG : HOLD_MS,
    );
  }, []);
      ...
      <AnimatePresence mode="wait">
        {shown && (
          <motion.div
            ...
            onMouseEnter={hold}
            onFocusCapture={hold}
            onMouseLeave={() => release(shown.big)}
            onBlurCapture={() => release(shown.big)}
