...
/** Eleven rules make twelve columns. Named rather than indexed, because a
 *  `key` taken from an array index is the thing that quietly breaks a list
 *  the first time anybody reorders it. */
const GRID_COLUMNS = [
  "c01",
  "c02",
  "c03",
...
/** What happens, in three words, numbered. The mono voice is reserved for
 *  metadata on this page, and an index is metadata. */
const HERO_INDEX = [
  { number: "01", label: "Speak" },
  { number: "02", label: "Marked" },
  { number: "03", label: "Gaps" },
] as const;

/** The masthead strip along the foot of the hero. Three facts somebody wants
 *  before they press the button, ruled apart — a band of information rather
 *  than a border with nothing in it. */
const HERO_FACTS = [
  { label: "Length", value: "Three minutes" },
  { label: "Notes", value: "None, you talk" },
  { label: "Audio", value: "Never stored" },
] as const;
  ...
  return (
    ...
    >
      ...
      <section
        id="hero"
        ref={heroRef}
        className="relative flex min-h-[min(88svh,52rem)] flex-col justify-center overflow-hidden bg-[color:var(--panel-deep)] px-5 pt-24 pb-0 text-primary-foreground md:px-10 lg:px-16"
      >
        {/* Structure, not a blue rectangle.
         *
         * The first version of this was a flat field with a very large
         * grotesk headline sitting in the middle of it, and the verdict from
         * three people who had never seen the page before was "just a blue
         * background, looks awkward". They were right, and the reason is
         * worth writing down: it is the only section on this page built out
         * of nothing. Every other one is ruled — hairlines, crop marks, a
         * mono slug, a ledger of columns — and this one had a headline
         * floating in a void, which is what a page looks like before anybody
         * has art-directed it.
         *
         * So the hero gets the page's own apparatus. A twelve-column rule
         * grid at four per cent white, drawn edge to edge; crop marks at the
         * corners of the type block; a numbered index down the right, set in
         * the mono voice this page reserves for metadata; and a ruled band
         * along the bottom carrying the three facts somebody actually needs
         * before they press the button. The type now sits *on* something.
         *
         * None of this is decoration in the sense `design.md` bans. There are
         * no orbs, no blur, no gradient standing in for an image and no
         * second hue. It is one hairline at one value, repeated — the same
         * device the rest of the page is made of, finally used in the place
         * that needed it most. */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 z-[1] hidden grid-cols-12 md:grid"
        >
          {GRID_COLUMNS.map((column) => (
            <span key={column} className="border-white/[0.045] border-l" />
          ))}
        </div>

        <div className="relative z-[2] mx-auto w-full max-w-[86rem] flex-1 flex-col justify-center md:flex">
          <div className="relative pt-8 pb-10 md:pt-10 md:pb-14">
            <CornerMarks tone="text-white/20" />
            <div className="flex items-start justify-between gap-10">
              <div className="min-w-0 flex-1">
                <p
                  data-hero-secondary
                  className="font-mono text-[0.64rem] text-primary-foreground/45 uppercase tracking-[0.2em]"
                >
                  Rehearse out loud
                </p>

                <h1 className="mt-7 font-display text-[clamp(2.4rem,7.6vw,7.4rem)] leading-[0.94] tracking-[-0.05em] md:mt-9">
                  ...
                  <span data-hero-word className="mt-1 block">
                    See what you{" "}
                    {/* The blank and the word share one box, so nothing
                        re-wraps when the word arrives — a headline that
                        re-flows mid-animation is a layout shift wearing a
                        costume. */}
              ...
              {/* The index. Three words for what happens, numbered, in the
                  voice this page keeps for metadata — and it doubles as the
                  thing that stops the headline from being alone on an empty
                  field. */}
              <ol
                data-hero-secondary
                className="hidden w-[9.5rem] shrink-0 border-white/12 border-t lg:block"
              >
                {HERO_INDEX.map((entry) => (
                  <li
                    key={entry.label}
                    className="flex items-baseline gap-3 border-white/12 border-b py-3"
                  >
                    <span className="font-mono text-[0.6rem] text-primary-foreground/35 tabular-nums">
                      {entry.number}
                    </span>
            ...
            <div className="mt-10 flex flex-col gap-7 md:mt-12 md:flex-row md:items-center md:justify-between md:gap-16">
              <p
                data-hero-secondary
                className="max-w-[34ch] text-[1.02rem] text-primary-foreground/70 leading-relaxed"
              >
                Every sentence is checked against your own material while you
                are still talking.
              </p>
              <div
                data-hero-secondary
              ...
              >
                <Link
                  href="/signup"
                  data-gsap-hover
                  className="group inline-flex h-12 items-center justify-center gap-4 border border-[var(--accent-solid)] bg-[var(--accent-solid)] px-8 font-medium text-[var(--brand-foreground)]"
                >
                  Start explaining
                ...
                <a
                  href="#live-demo"
                  data-gsap-hover
                  className="lp-nav-link inline-flex h-12 items-center justify-center px-1 font-medium text-primary-foreground/70"
                >
        ...
        {/* The masthead strip. Ruled, divided, and carrying the three things
            somebody wants to know before they sign up — which is what makes
            it a band of information rather than a border. */}
        <div
          data-hero-secondary
          className="relative z-[2] mx-auto grid w-full max-w-[86rem] border-white/12 border-t sm:grid-cols-3"
        >
          {HERO_FACTS.map((fact, index) => (
            <div
              key={fact.label}
              className={`flex items-baseline gap-3 py-5 ${
                index === 0 ? "" : "sm:border-white/12 sm:border-l sm:pl-8"
              }`}
            >
              <span className="font-mono text-[0.62rem] text-primary-foreground/40 uppercase tracking-[0.16em]">
                {fact.label}
              </span>
              <span className="ml-auto font-medium text-[0.95rem] sm:ml-0">
                {fact.value}
              </span>
            </div>
          ))}
