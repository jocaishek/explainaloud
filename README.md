...
- **The three verdict colours are reserved.** `--ok`, `--miss` and `--vague`
  ...
  in afterwards.
- **One accent, one hue.** A single warm ramp — `#b45309` deep, `#f59e0b`
  solid, `#fbbf24` high — and a second hue introduced as scenery is how a
  reserved palette stops being reserved. Ask for `--brand`, `--brand-deep`,
  `--brand-high` or `--brand-ink` and never a literal; the whole product
  changed accent three times in one afternoon because those names exist.
  The one constraint on moving along this ramp is `--vague` (`#96600a`), which
  means "said, but not checkably" and is the same hue family — nothing may
  plot brand against vague and expect the difference to read.
