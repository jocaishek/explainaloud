 ...
 */
/* Jagged on purpose: smooth up-and-down ramps read as a row of little
 * pyramids, not as audio. A real voice note jumps — loud syllable, quiet
 * one, spike, breath — so no run here climbs or falls more than two steps
 * in a row. */
const HERO_WAVE = [
  22, 58, 34, 76, 28, 50, 90, 40, 18, 64, 46, 82, 30, 56, 24, 70, 44, 92, 36,
  60, 20, 48, 78, 32, 66, 26, 54, 86, 42, 16, 62, 38, 74, 28, 52, 94, 34, 68,
  22, 58, 44, 80, 30, 14, 48, 72, 26, 56, 88, 38, 64, 20, 46, 76, 32, 60, 24,
  52, 84, 40, 68, 28, 16, 50, 36, 62, 22, 44, 30, 54, 18, 34,
] as const;

/* The play-through delay, then a personal phase and period for the idle
   sway, so the resting strip breathes like a quiet room rather than pulsing
   like one mechanism. */
const HERO_BARS = HERO_WAVE.map((height, index) => ({
  height,
  at: 520 + index * 33,
  idleAt: 520 + index * 33 + 700 + ((index * 137) % 1100),
  idleDur: 2800 + (index % 5) * 340,
}));

/**
 * The take, hanging in the air.
 *
 * Fragments of a spoken explanation around the headline, each lighting as it
 * is "said" and taking its verdict a beat later — the first screen becomes
 * the thing the product does: words in the air, being judged. They are the
 * ticker's old self-describing lines, the one demonstration this page is
 * allowed: claims about the product, marked as the grader marks them.
 *
 * Positioned in rem against the *content block*, never as viewport
 * percentages — the first cut used `top: 58%` of the hero and on a taller
 * window than the one it was tuned in, the fragments drifted into the
 * waveform. Anchored to the block, they hold their clearance at every
 * aspect. Six, not ten: scenery whispers. Only two render under `md`, in
 * the bands above the headline and below the CTA, and the whole layer is
 * `aria-hidden` — a screen reader gets the headline instead.
 */
const TAKE_FRAGMENTS: Array<{
  text: string;
  verdict: "ok" | "vague" | "miss";
  pos: string;
  size: string;
  at: number;
}> = [
  {
    text: "three minutes, nothing scripted",
    verdict: "ok",
    pos: "top-[-5.5rem] left-0 max-md:top-[-4rem]",
    size: "1.05rem",
    at: 750,
  },
  {
    text: "checked claim by claim",
    verdict: "ok",
    pos: "hidden md:block top-[-4.25rem] right-0",
    size: "1.15rem",
    at: 1150,
  },
  {
    text: "the step you skipped",
    verdict: "miss",
    pos: "hidden md:block top-[58%] left-0",
    size: "1.1rem",
    at: 1600,
  },
  {
    ...
    verdict: "vague",
    pos: "hidden md:block top-[58%] right-0",
    size: "1rem",
    at: 2050,
  },
  {
    ...
    verdict: "ok",
    pos: "hidden md:block bottom-[-4.5rem] left-[8%]",
    size: "1.05rem",
    at: 2500,
  },
  {
    text: "never turns up at all",
    verdict: "miss",
    pos: "bottom-[-6rem] right-[4%] max-md:bottom-[-4.75rem] max-md:right-auto max-md:left-0",
    size: "1.2rem",
    at: 2950,
  },
];
    ...
    >
      ...
      >
        ...
        <div className="relative z-[2] flex min-h-screen flex-col pt-20 md:pt-24 lg:py-24">
          {/* The hero speaks itself.
              Eight words, no panel — but a still headline with two coloured
              words was, fairly, "just text". The moat in PRODUCT.md is live
              colouring while you speak, so the headline performs it: the
              words brighten in reading order at speaking pace, the way live
              captions light, a speech-shaped waveform runs underneath, and
              each verdict lands the moment its word is said — "know." goes
              green mid-sentence, "missed." goes red at the end. Then it
              rests, fully marked. One orchestrated moment, no loop.

              This is not the retired typing hero: every word is present from
              first paint at readable opacity, nothing reflows, nothing
              resets. It is the karaoke gesture, not the typewriter one.

              Pure CSS with `both` fills throughout: before its delay a word
              is dim, after it, said forever; if scripting dies the page
              still ends fully lit and marked — the resting-state lesson the
              streak flame taught. */}
          <div className="relative z-10 mx-auto flex w-full max-w-[76rem] flex-1 flex-col items-center justify-center py-4 text-center">
            {/* Hugs the content, so the constellation's rem offsets mean
                "above the headline" and "below the CTA" at every viewport —
                anchored to the stretched wrapper they meant "above the hero",
                which clipped the top band and pinned the bottom one to the
                fold. */}
            <div className="relative flex w-full flex-col items-center">
              {/* The graded air, anchored to this block. See TAKE_FRAGMENTS. */}
              <div
                aria-hidden="true"
                className="pointer-events-none absolute inset-0"
              >
                {TAKE_FRAGMENTS.map((frag) => (
                  <span
                    key={frag.text}
                    className={`lp-frag absolute ${frag.pos}`}
                    data-verdict={frag.verdict}
                    style={{
                      fontSize: frag.size,
                      animationDelay: `${frag.at}ms`,
                    }}
                  >
                    {frag.text}
                  </span>
              ...
              <h1 className="flex flex-col items-center gap-[0.16em] font-display text-[clamp(2.6rem,7.6vw,7.5rem)] text-primary-foreground leading-[0.98] tracking-[-0.035em]">
                <span data-hero-word className="block">
                  <span className="lp-say" style={{ animationDelay: "650ms" }}>
                    Say
                  </span>{" "}
                  <span className="lp-say" style={{ animationDelay: "870ms" }}>
                    what
                  </span>{" "}
                  <span className="lp-say" style={{ animationDelay: "1090ms" }}>
                    you
                  </span>{" "}
                  <span className="lp-say" style={{ animationDelay: "1310ms" }}>
                    <span
                      className="lp-grade"
                      data-verdict="ok"
                ...
                <span data-hero-word className="block">
                  <span className="lp-say" style={{ animationDelay: "1900ms" }}>
                    See
                  </span>{" "}
                  <span className="lp-say" style={{ animationDelay: "2120ms" }}>
                    what
                  </span>{" "}
                  <span className="lp-say" style={{ animationDelay: "2340ms" }}>
                    you
                  </span>{" "}
                  <span className="lp-say" style={{ animationDelay: "2560ms" }}>
                    <span
                      className="lp-grade"
                      data-verdict="miss"
              ...
              {/* The take's waveform: speech-shaped, played through once on
                the same clock the words light to, resting as a quiet strip.
                It is what says "out loud" without a word of copy — the same
                voice as the waveform in the nav, at the hero's scale. */}
              <div
                aria-hidden="true"
                data-hero-secondary
                className="mt-7 flex h-12 w-full max-w-[42rem] items-center justify-center gap-[3px] px-4 md:h-16"
              >
                {HERO_BARS.map((bar) => (
                  <span
                    key={bar.at}
                    ...
                    style={{
                      height: `${bar.height}%`,
                      animationDelay: `${bar.at}ms, ${bar.idleAt}ms`,
                      animationDuration: `520ms, ${bar.idleDur}ms`,
                    }}
                  />
                ))}
              </div>
              ...
              <p
                data-hero-secondary
                className="mt-6 max-w-[34rem] text-[1.05rem] text-primary-foreground/80 leading-relaxed"
              >
