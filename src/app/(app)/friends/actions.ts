...
export type SquadActionResult = { ok: boolean; message: string };

/**
 * Starts a squad and puts you in it.
 *
 * The name is the only thing the caller supplies. The join code is generated
 * in the database — a code somebody can choose is a code somebody can squat
 * on, or guess at — and the membership row is written by the same function, so
 * there is no window in which a squad exists with nobody in it.
 */
export async function createSquad(name: string): Promise<SquadActionResult> {
  const { supabase } = await requireUser();
  const trimmed = name.trim();

  if (trimmed.length < 2 || trimmed.length > 40) {
    return { ok: false, message: "Give it a name of 2 to 40 characters." };
  }
  ...
  if (error) {
    console.error("Creating a squad failed:", error);
    return {
      ok: false,
      message: error.message.includes("too many squads")
        ? "You're in as many squads as we allow. Leave one first."
        : "Couldn't start that squad. Try again.",
    };
  }

  refresh();
  return { ok: true, message: "Squad started." };
}

/**
 * Joins one by its code.
 *
 * A wrong code comes back as a null rather than as an error, because it is not
 * one: mistyping six characters read off somebody's screen is the expected
 * case, and it deserves a sentence rather than a failure.
 */
export async function joinSquad(code: string): Promise<SquadActionResult> {
  const { supabase } = await requireUser();
  const cleaned = code.trim().toUpperCase();

  if (!/^[BCDFGHJKLMNPQRSTVWXYZ23456789]{6}$/.test(cleaned)) {
    return { ok: false, message: "A join code is six letters and numbers." };
  }

  const { data, error } = await supabase.rpc("join_squad", { code: cleaned });

  if (error) {
    console.error("Joining a squad failed:", error);
    return {
      ok: false,
      message: error.message.includes("full")
        ? "That squad is full."
        : error.message.includes("too many squads")
          ? "You're in as many squads as we allow. Leave one first."
          : "Couldn't join that squad. Try again.",
    };
  }

  if (!data) {
    return { ok: false, message: "No squad has that code." };
  }
...
/**
 * Leaves one — or closes it, if you started it.
 *
 * Handing ownership to whoever joined next is the other option and it is
 * worse: it makes somebody responsible for a group they did not start,
 * silently, on a day nobody asked them.
 */
export async function leaveSquad(squadId: string): Promise<SquadActionResult> {
  const { supabase } = await requireUser();

  if (!UUID.test(squadId)) {
    return { ok: false, message: "That isn't a squad we can find." };
  }

  const { error } = await supabase.rpc("leave_squad", { target: squadId });

  if (error) {
    console.error("Leaving a squad failed:", error);
    return { ok: false, message: "Couldn't leave that squad. Try again." };
  }
