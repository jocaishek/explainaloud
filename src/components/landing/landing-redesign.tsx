...
function scaleFlow(hero: HTMLElement) {
  ...
  /* Where the type ends, in the hero's own pixels. The wave passes under it,
     and on a frame that is wide but short there is not much room below the
     buttons to pass in — the drawing is scaled by the width and the column is
     not, so the two close on each other. The wave is pushed down far enough
     to clear the trust line. */
  ...
  for (const svg of hero.querySelectorAll<SVGSVGElement>(".lp-flow")) {
    ...
    if (path) {
      ...
      if (path.dataset.anchor === "bottom") {
        /* ...but not so far that it falls out of the bottom of the frame.
           The words sit *on* the path, so the last few pixels of a push get
           paid for by the descenders and then by the whole line: the wave
           bleeds off the sides, which is the drawing, and had started
           bleeding off the foot, which is a clipped sentence. Where the two
           bounds disagree — a frame too short to hold both — staying in the
           frame wins, because a line cut in half is worse than a line
           passing close. */
        const lowest = height - FLOW_FOOT * scale - lowestOf(design) * scale;
        const clear = floor + FLOW_CLEAR - highestOf(design) * scale;
        offset = Math.min(lowest, Math.max(drop, clear));
      }
      path.setAttribute("d", scalePath(design, scale, offset));

      /* The grading ramp runs the length of the stream, not the width of the
         page.
         *
         * It used to be laid across the frame, which is right for the wave —
         * it crosses the whole page, so crossing the frame's ramp is crossing
         * its own. It is wrong for anything that does not. The top right
         * curve lives in the last third of the width, entirely inside the
         * grey end of a frame-wide ramp, so its words were grey from the
         * moment they appeared to the moment they left: a marked take that
         * never showed a mark. Anchoring the ramp to the curve's own box
         * gives every stream the same gesture over its own run. */
      const box = path.getBBox();
      for (const ramp of svg.querySelectorAll("[data-flow-grad]")) {
        ramp.setAttribute("x1", `${box.x.toFixed(1)}`);
        ramp.setAttribute("x2", `${(box.x + box.width).toFixed(1)}`);
      }
    }
  }
  return scale;
...
/** And how much it keeps under itself, in design units, for descenders. */
const FLOW_FOOT = 26;
...
function highestOf(d: string) {
  return edgeOf(d, Math.min, Number.POSITIVE_INFINITY);
}

/** And its lowest. */
function lowestOf(d: string) {
  return edgeOf(d, Math.max, Number.NEGATIVE_INFINITY);
}
...
function edgeOf(
  ...
  for (let n = 1; n < numbers.length; n += 2) {
    edge = pick(edge, Number.parseFloat(numbers[n] as string));
  }
