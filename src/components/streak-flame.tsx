 ...
 *
 * **Two arrivals, because there are two situations.** `arrive` rises in from
 * nothing and is for a flame that appears with the thing carrying it — the
 * streak notice, which did not exist a moment ago. `catch` lights in place and
 * is for a flame already on the page that a reader has just scrolled to.
 *
 * The distinction is not stylistic. `arrive` begins at zero opacity, so
 * anything using it has to be absent until it plays; a flame that is already
 * drawn would blink out and come back. `catch` never touches opacity, which
 * means the element is never hidden waiting for a trigger — and that matters,
 * because the trigger is an IntersectionObserver, and an observer in a
 * background or occluded tab does not fire at all. Hiding something pending an
 * event that may not arrive is how a decoration becomes a blank space.
 */
export function StreakFlame({
  ...
  lit?: boolean;
  /** `arrive` rises in from nothing; `catch` lights a flame already on screen. */
  arrival?: "arrive" | "catch";
  className?: string;
  size?: "sm" | "md" | "lg" | "xl";
}) {
  return (
    <span
      aria-hidden
      data-lit={lit ? arrival : undefined}
      className={cn(
