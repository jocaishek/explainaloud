...
/**
 * A real take, marked the way the grader marks it.
 *
 ...
 * would mark it. It is written the way somebody actually talks when they are
 * explaining something from memory, hedges and all, because a transcript of
 * perfect prose is not a transcript of anybody speaking.
 *
 * `ok` is a claim that landed. `vague` was said but not in a way anything
 * could be checked against. `miss` is not speech at all — it is the grader
 * naming a step that was never reached, which is why those lines read as an
 * interruption rather than as more of the take.
 */
const HERO_TAKE_LINES = [
  {
    verdict: "ok",
    text: "Mitosis starts when the cell makes a full copy of its DNA.",
  },
  {
    verdict: "ok",
    text: "So every chromosome ends up as two identical halves joined in the middle.",
  },
  {
    verdict: "miss",
    text: "the two halves stay attached at the centromere until anaphase.",
  },
  {
    verdict: "ok",
    text: "Then the envelope around the nucleus breaks apart and the fibres come in.",
  },
  {
    verdict: "vague",
    text: "They line everything up and then pull it to opposite ends.",
  },
  {
    verdict: "miss",
    text: "each new cell ends up with the full chromosome count, not half.",
  },
  {
    verdict: "ok",
    text: "So at the end you get two cells identical to the one you started with.",
  },
  {
    verdict: "ok",
    text: "Which is the difference from meiosis, because that one halves it.",
  },
  {
    verdict: "vague",
    text: "And the whole thing happens pretty fast, like a few hours.",
  },
] as const;
  ...
  return (
    ...
    >
      ...
      <section
        id="hero"
        ref={heroRef}
        className="relative overflow-hidden border-border border-b bg-background px-5 pt-24 pb-16 md:px-10 md:pb-20 lg:px-16"
      >
        {/* The take is the picture.
         *
         * Four heroes were built before this one and every one of them was
         * dark, restrained and typographic: a console, a strip of marks, a
         * headline on a flat navy field, and then the same headline on a
         * ruled navy field. The verdicts on the last two, from people who had
         * never seen the page: "just a blue background", "bland", "nothing
         * unique". Four is enough to stop treating it as a series of near
         * misses and start treating it as the direction being wrong.
         *
         * The thing this product makes is a page of somebody's own speech
         * with colour on it. That is the most distinctive object it owns and
         * it had never been on the landing page at full size — it had been
         * abstracted into a bar chart, hidden inside a fake application
         * window, or left out. Here it is the artwork: a real take, marked,
         * set at reading size, bleeding off the right edge so it reads as an
         * excerpt of something longer rather than as a card.
         *
         * It is on the light stock, which is a change of register and a
         * deliberate one. `design.md`: a section that marks anything takes a
         * light ground, because green, tan and red have to stay legible and
         * keep meaning *correct*, *too thin* and *never reached*. The page's
         * single inversion belongs at the close, where going dark says this
         * is the end — and it was being spent at the top, where it bought a
         * blue rectangle. */}
        {/* Absolute, at the very top. The bar reads this to decide whether it
            is over dark stock — and the hero is light now, so the answer has
            to be yes on the first painted frame. Left in the flow it sat
            below the section's padding and the bar spent the first screen
            navy over paper. */}
        <div
          ref={navSentinelRef}
          aria-hidden="true"
        ...
        <div className="mx-auto grid w-full max-w-[86rem] items-center gap-14 lg:grid-cols-[1.02fr_0.98fr] lg:gap-16">
          <div className="max-w-[36rem] lg:max-w-none">
            <p
              data-hero-secondary
              className="font-mono text-[0.64rem] text-brand-ink uppercase tracking-[0.2em]"
            >
              Rehearse out loud &middot; three minutes
            </p>

            <h1 className="mt-7 font-display text-[clamp(2.3rem,5.4vw,4.9rem)] text-strong leading-[0.95] tracking-[-0.05em] md:mt-9">
              ...
              <span data-hero-word className="mt-1 block">
                See what you{" "}
                {/* The blank and the word share one box, so nothing re-wraps
                    when the word arrives — a headline that re-flows
            ...
            <p
              data-hero-secondary
              className="mt-7 max-w-[34ch] text-[1.05rem] text-muted-foreground leading-relaxed"
            >
              You explain it in your own words. Every sentence is checked
              against your own material while you are still talking.
            </p>

            <div
              data-hero-secondary
              className="mt-9 flex w-full flex-col items-stretch gap-4 sm:w-auto sm:flex-row sm:items-center"
            >
              <Link
                href="/signup"
                data-gsap-hover
                className="group inline-flex h-12 items-center justify-center gap-4 bg-[var(--panel-deep)] px-8 font-medium text-primary-foreground"
              >
                Start explaining
              ...
              <a
                href="#live-demo"
                data-gsap-hover
                /* Not `lp-nav-link`. That class carries the bar's own ink —
                   a near-white tuned for navy — so on paper the link was
                   white on white and simply was not there. */
                className="group inline-flex h-12 items-center justify-center gap-2 px-1 font-medium text-brand-ink"
              >
          ...
          {/* The wall. Bleeds off the right edge on desktop, because a take
              that stops neatly inside a box is a card, and a take that runs
              off the page is a page. */}
          <div
            data-hero-secondary
            className="lp-wall relative border-border border-t pt-8 lg:-mr-10 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-14 xl:-mr-16"
          >
            <div className="mb-6 flex items-baseline justify-between font-mono text-[0.6rem] text-muted-foreground uppercase tracking-[0.16em]">
              <span>Rehearsal &middot; biology</span>
              <span className="tabular-nums">02:41</span>
            </div>

            <div className="lp-wall-body">
              {HERO_TAKE_LINES.map((line, index) => (
                <p
                  key={line.text}
                  className="lp-wall-line"
                  /* `data-mark`, not `data-verdict`. The page-wide sweep
                     claims every `[data-verdict]` it can find, splits it into
                     one span per character and re-colours the lot — which
                     turned the whole take into tracked-out green confetti the
                     first time this shipped. That sweep is for a phrase inside
                     a headline; this is a transcript. */
                  data-mark={line.verdict}
                  style={{ animationDelay: `${900 + index * 130}ms` }}
                >
                  <span aria-hidden="true" className="lp-wall-tick" />
                  {line.verdict === "miss" ? (
                    <span>
                      <b className="font-semibold">Never said —</b> {line.text}
                    </span>
                  ) : (
                    <span>{line.text}</span>
                  )}
