...
/**
 * The same idea, redrawn for a narrow frame.
 *
 * The three landscape streams are composed in a 1440x900 space, and `slice`
 * on a 500-wide window crops that to the middle quarter — three curves become
 * three unrelated diagonals and the type scales down with them. They hold
 * their composition a long way down, but not all the way, so below the
 * breakpoint the hero says the same two things in a drawing of its own: two
 * takes on a shallow wave, in the two bands the centred column leaves empty,
 * above the headline and below the buttons. The marked take keeps the lower
 * band, because it is the one that shows what the product does.
 *
 * No path here: `FlowWave` solves for its own, from the width of the type it
 * is given. What this list carries is what is said and when it starts.
 */
...
function FlowWave({
  ...
  /* Bumped on every rebuild, and used as the belt's React key.
   *
   * A CSS animation starts when its element does, so a rebuild that *edits*
   * the belt — a resize asking for two more copies of the take, say — leaves
   * the words that were already there running on the old clock and starts the
   * new ones from zero. They then run permanently out of phase, and since
   * they are copies of the same sentence at the same distances the page shows
   * two takes overprinted on each other. Keying the whole belt makes a
   * rebuild atomic: every word is new, so every word starts together. */
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setBelt({
        d: wavePath(span, wavelength, amp, mid),
        height,
        ...
        /* Where the grading happens, in the rail's own pixels. The band's
           left edge is one unit along the rail, and the crossing sits in the
           same place across the frame as the desktop curves put it. */
        markUntil: unit + band.clientWidth * 0.44,
        greyFrom: unit + band.clientWidth * 0.78,
      });
    };
  ...
  return (
    ...
    >
      ...
      {belt ? (
        ...
        >
          {/* Twice, and the pair is the marking.
           *
           * A word has to arrive grey and be in its verdict colour by the
           * time it is halfway across — the mark happening *to* it while it
           * travels, which is the product's own gesture and the one thing the
           * straight band gave up. The colour therefore belongs to the place
           * on the page, not to the word, and a word carrying its own colour
           * cannot be painted by the frame it is passing through.
           *
           * So the belt is laid twice — once in the stream's grey, once in
           * the verdicts — and the marked copy is masked to the left of the
           * frame. What shows through on the right is the grey one. Both are
           * the same words at the same distances on the same rail, started in
           * the same frame, so they travel as one; the reader sees a single
           * belt that changes colour as it crosses. The desktop curves do
           * exactly this with a gradient in user space. */}
          {(["grey", "mark"] as const).map((coat) => (
            <div
              key={coat}
              className={`lp-band-rail lp-band-rail-${coat}`}
              style={
                coat === "mark"
                  ? ({
                      "--mark-until": `${belt.markUntil.toFixed(1)}px`,
                      "--grey-from": `${belt.greyFrom.toFixed(1)}px`,
                    } as CSSProperties)
                  : undefined
              }
            >
              {belt.rungs.map((rung, copy) =>
                words.map((word, n) => {
                  ...
                  return (
                    <span
                      key={`${coat}-${rung}-${word.key}`}
                      className="lp-band-word"
                      data-mark={
                        coat === "mark" && word.verdict !== "none"
                          ? word.verdict
                          : undefined
                      }
                      style={
...
type Belt = {
  ...
  rungs: string[];
  duration: number;
  /** Which rebuild this is. The belt's key, so a rebuild starts one clock. */
  build: number;
  /** How far across the rail a word is still fully in its verdict colour. */
  markUntil: number;
  /** And where it has finished turning back to the stream's grey. */
  greyFrom: number;
};

/** Samples per wavelength when adding up how long a wave is. */
const WAVE_SAMPLES = 128;

/** Cubic segments per wavelength when drawing one. */
const WAVE_SEGMENTS = 8;

/**
 * The rail: a sine, as cubic curves rather than as a polyline.
 *
 * The polyline this replaced was accurate to a tenth of a pixel in *position*
 * and badly wrong in *direction*. A word on a motion path takes its angle
 * from the tangent under it, and a polyline's tangent is constant along a
 * segment and then jumps — so a word crossing a vertex snapped a degree or
 * two, and a long word's tail moved a pixel and a half in one frame. Two
 * dozen of those per wavelength is not a wave, it is a flicker, and it is
 * what "the words jump" was.
 *
 * A cubic per eighth of a wavelength, with its control points set from the
 * true derivative at each end, matches the sine's slope exactly at every
 * join. The tangent is continuous, so the type turns as smoothly as it
 * travels.
 */
function wavePath(span: number, wavelength: number, amp: number, mid: number) {
  const k = (2 * Math.PI) / wavelength;
  const at = (x: number) => mid + amp * Math.sin(k * x);
  const slope = (x: number) => amp * k * Math.cos(k * x);

  /* The span is always a whole number of wavelengths, so every segment is the
     same length and the count can be counted rather than accumulated. */
  const segments = Math.max(1, Math.round((span / wavelength) * WAVE_SEGMENTS));
  const step = span / segments;
  /* A third of the way along the segment is where a cubic's control point has
     to sit for its handle to *be* the derivative there. */
  const reach = step / 3;

  let d = `M0,${at(0).toFixed(2)}`;
  for (let i = 0; i < segments; i += 1) {
    const x0 = i * step;
    const x1 = (i + 1) * step;
    d +=
      `C${(x0 + reach).toFixed(2)},${(at(x0) + slope(x0) * reach).toFixed(2)}` +
      ` ${(x1 - reach).toFixed(2)},${(at(x1) - slope(x1) * reach).toFixed(2)}` +
      ` ${x1.toFixed(2)},${at(x1).toFixed(2)}`;
  }
  return d;
}
