...
/**
 * Fragments of real takes, marked, for the band along the foot of the hero.
 *
 * Deliberately from four different subjects. Everything else above the fold
 * is one biology rehearsal, and the one thing a single example cannot say is
 * that this happens to whatever you are studying. Authored, as all the
 * demonstration material here is, and marked exactly as the grader marks it.
 */
const HERO_TICKER = [
  { verdict: "ok", text: "mitosis copies the DNA before anything splits" },
  { verdict: "miss", text: "never said: they stay joined at the centromere" },
  { verdict: "ok", text: "the New Deal came in two waves" },
  { verdict: "vague", text: "it happened pretty fast" },
  { verdict: "ok", text: "a derivative is a rate of change" },
  { verdict: "miss", text: "never said: at one single point" },
  { verdict: "ok", text: "supply shifts and price follows it" },
  { verdict: "vague", text: "so it kind of balances out" },
  { verdict: "ok", text: "photosynthesis needs light and carbon dioxide" },
  { verdict: "miss", text: "never said: where the oxygen comes from" },
] as const;

/* Twice through, so the loop has no seam. The pass is part of the key rather
   than the position, because a key taken from an array index is the thing
   that quietly breaks a list the first time anybody reorders it. */
const HERO_TICKER_LOOP = ["a", "b"].flatMap((pass) =>
  HERO_TICKER.map((item) => ({ ...item, key: `${pass}-${item.text}` })),
);
  ...
  return (
    ...
    >
      ...
      <section
        id="hero"
        ref={heroRef}
        className="relative flex min-h-[min(90svh,54rem)] flex-col overflow-hidden border-border border-b bg-background px-5 pt-24 pb-0 md:px-10 lg:px-14"
      >
        {/* A poster, and the redaction is the product.
         *
         * Five heroes came before this one and they were all the same
         * instinct: restrained, mid-sized type, an object beside it to prove
         * the point. A console, a bar chart, a ruled field, a marked
         * transcript. The verdict every time, from people seeing it cold, was
         * some version of bland.
         *
         * The correction is scale and a single hard idea. The type runs the
         * full measure of the page at poster size, and the last word is
         * blacked out. A redaction bar is the most legible picture of a gap
         * anybody has, it needs no key and no caption, and it happens to be
         * exactly what this product hands you at the end of three minutes. It
         * lifts, left to right, and the word underneath is the one the page
         * has been about the whole time.
         *
         * The word is in the markup and never hidden by opacity, only covered
         * by a block that is `aria-hidden` and animated with a clip. So a
         * reader with no CSS animation, or with reduced motion, gets the
         * finished sentence rather than a black rectangle. */}
        <div className="relative z-[2] mx-auto flex w-full max-w-[92rem] flex-1 flex-col justify-center py-8">
          <div
            data-hero-secondary
          ...
          >
            ...
            <span className="text-right">Three minutes &middot; no notes</span>
          </div>

          <h1
            data-hero-word
            className="mt-7 font-display text-[clamp(2.3rem,9.4vw,8.6rem)] text-strong uppercase leading-[0.88] tracking-[-0.055em] md:mt-9"
          >
            {/* Two lines, not three. "And find the gaps." wrapped the
                redaction onto a line of its own at every viewport under about
                1700px, and a bar sitting alone under a sentence is a bar; a
                bar at the end of the sentence it belongs to is the sentence
                having something taken out of it. */}
            {/* Green on the doing, red on the finding. Two thirds of the
                headline was flat ink and the page had almost no colour above
                the fold; these are the two words the reserved palette exists
                for, so they take it. */}
            ...
            <span className="block whitespace-nowrap">
              ...
              <span className="lp-redact">
                ...
                {/* The one annotation, hung off the redaction itself rather
                    than set under the line. A caption that refers to a word
                    has to sit under that word: under the line it landed
                    beneath "find the", six hundred pixels from the thing it
                    was labelling. */}
                <span aria-hidden="true" className="lp-redact-note font-mono">
                  The part you never reached
                </span>
              </span>
            </span>
          </h1>

          <div className="mt-11 flex flex-col gap-8 border-border border-t pt-8 md:mt-14 md:flex-row md:items-end md:justify-between md:gap-16">
            <p
              data-hero-secondary
              className="max-w-[46ch] text-[1.05rem] text-muted-foreground leading-relaxed md:text-[1.12rem]"
            >
              You explain the topic in your own words, with nothing to read off.
              Every sentence is checked against your own material while you are
              still talking, and what you never reached is the part worth
              knowing.
            </p>
            <div
              data-hero-secondary
        ...
        {/* A live stream of marks along the foot.
         *
         * The poster was two lines of type on a flat field with a lot of
         * empty page under it, and the note back was that it looked plain.
         * This is the cheapest honest way to fill that band: real marked
         * fragments from real takes, in the three colours that mean
         * something, moving slowly and continuously.
         *
         * It is not decoration in the sense `design.md` bans. It carries
         * content, the content is the product's own output, and it is the
         * only thing on the page that says this happens across every
         * subject rather than to one biology take. It also gives the first
         * screen something that moves without anything being attached to
         * the pointer or driven by scroll.
         *
         * Full bleed, so it reads as a band rather than as a wide card, and
         * the row is duplicated so the loop has no seam. */}
        <div aria-hidden="true" className="lp-ticker -mx-5 md:-mx-10 lg:-mx-14">
          <div className="lp-ticker-row">
            {HERO_TICKER_LOOP.map((item) => (
              <span
                key={item.key}
                className="lp-ticker-item"
                data-mark={item.verdict}
              >
                <i />
                {item.text}
              </span>
